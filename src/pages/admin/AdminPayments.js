...
import { Wallet, AlertCircle, FileText, Search, Loader2, X, Eye, Check, RefreshCw, Calendar, Clock, Stethoscope } from 'lucide-react';
...
export default function AdminPayments() {
  ...
  const modalAppt = (selectedPayment && Array.isArray(appointments) && typeof appointments.find === 'function')
    ? appointments.find(appt => appt.id === selectedPayment.appointment_id)
    : null;
  const modalApptDoctorName = modalAppt
    ? (isRtl
      ? (modalAppt.doctor_name_ar || modalAppt.doctor?.name_ar || modalAppt.doctor_name || modalAppt.doctor?.name)
      : (modalAppt.doctor_name || modalAppt.doctor?.name_en || modalAppt.doctor?.name))
    : '';
  const modalApptSpecialization = modalAppt ? (modalAppt.doctor_specialization || modalAppt.doctor?.specialization) : '';
  const modalApptScheduledAt = modalAppt ? (modalAppt.scheduled_at || modalAppt.scheduledAt || modalAppt.date) : null;
              ...
              {loading && payments.length === 0 ? (
                ...
                filteredPayments.map((p) => {
                  ...
                  const associatedAppt = (Array.isArray(appointments) && typeof appointments.find === 'function')
                    ? appointments.find(appt => appt.id === p.appointment_id)
                    : undefined;
                  const docName = p.doctorName || (associatedAppt
                    ? (isRtl
                      ? (associatedAppt.doctor_name_ar || associatedAppt.doctor?.name_ar || associatedAppt.doctor_name || associatedAppt.doctor?.name)
                      : (associatedAppt.doctor_name || associatedAppt.doctor?.name_en || associatedAppt.doctor?.name))
                    : '');
                  const apptDateStr = associatedAppt?.scheduled_at || associatedAppt?.scheduledAt || associatedAppt?.date;
                  const isFuture = apptDateStr ? new Date(apptDateStr) > new Date() : false;
                ...
                <div className="flex-1 overflow-y-auto space-y-5 pr-1">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest block">
                      {t('adminPayments.modal.requestedAppointment')}
                    </label>
                    {modalAppt ? (
                      <div className="bg-primary-50/60 p-4 rounded-2xl border border-primary-100 space-y-3">
                        <div className="flex items-center gap-3">
                          <div className="p-2 bg-white text-primary-600 rounded-xl border border-primary-100 shrink-0">
                            <Stethoscope className="w-4 h-4" />
                          </div>
                          <div className="min-w-0 flex-1">
                            <div className="text-sm font-bold text-slate-800 truncate">{modalApptDoctorName || t('adminPayments.doctorLabel')}</div>
                            {modalApptSpecialization && <div className="text-[11px] font-medium text-slate-500 truncate">{modalApptSpecialization}</div>}
                          </div>
                          <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider shrink-0 ${isFollowUpAppointment(modalAppt) ? 'bg-indigo-100 text-indigo-700' : 'bg-primary-100 text-primary-700'}`}>
                            {getApptTypeLabel(modalAppt, t)}
                          </span>
                        </div>
                        <div className="flex items-center gap-4 text-xs font-semibold text-slate-700">
                          <div className="flex items-center gap-1.5">
                            <Calendar className="w-3.5 h-3.5 text-primary-500" />
                            {modalApptScheduledAt ? formatDate(modalApptScheduledAt, isRtl, { dateStyle: 'medium' }) : 'N/A'}
                          </div>
                          <div className="flex items-center gap-1.5">
                            <Clock className="w-3.5 h-3.5 text-primary-500" />
                            {modalApptScheduledAt ? formatTime(modalApptScheduledAt, isRtl) : 'N/A'}
                          </div>
                        </div>
                        {modalAppt.notes && (
                          <div className="pt-2 border-t border-primary-100/70">
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">
                              {t('adminPayments.modal.reasonLabel')}
                            </span>
                            <p className="text-xs font-medium text-slate-600">{modalAppt.notes}</p>
                          </div>
                        )}
                      </div>
                    ) : (
                      <div className="bg-slate-50 p-4 rounded-2xl border border-dashed border-slate-200 text-xs font-bold text-slate-400 text-center">
                        {t('adminPayments.modal.noAppointmentLinked')}
                      </div>
                    )}
                  </div>
